...
- Name: recite-flow.spec.ts >> recite flow: expand text and self-evaluate correct
- Location: tests/e2e/recite-flow.spec.ts:39:1
...
Call log:
  ...
  - waiting for locator('text=查看原文')

```

```yaml
- heading "古诗背诵" [level=2]
- heading "背诵来源" [level=3]
- button "待复习"
- button "全部古诗"
- heading "选择年级（不选则为全部）" [level=3]
- button "开始背诵"
- button "返回首页"
```
...
```ts
  ...
  2   | 
  3   | // Helper: navigate to recite page
  4   | async function gotoRecitePage(page: any) {
  5   |   await page.goto('/')
  6   |   await page.evaluate(() => localStorage.clear())
  7   |   await page.reload()
  8   |   await page.click('text=自评背诵')
  9   | }
  10  | 
